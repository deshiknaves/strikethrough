...
<div class="flex flex-col overflow-y-auto p-4 min-[501px]:h-dvh min-[501px]:overflow-hidden">
	<h1 class="p-3 text-2xl font-bold">{heading}</h1>
	<div class="flex min-h-0 flex-1 gap-3">
		{#each weekdays as day (day.toString())}
			<div class="min-h-[200px] flex-1 rounded-lg border border-gray-200 p-3 min-[501px]:min-h-0">
				<h2 class="flex justify-between text-sm font-semibold"><span>{date(day)}</span><span class="text-gray-400">{weekday(day)}</span></h2>
			</div>
		{/each}
	</div>
	<div class="flex min-h-[150px] gap-3 min-[501px]:h-[35%] min-[501px]:min-h-0 mt-3">
		{#each weekend as day (day.toString())}
			<div class="w-1/2 rounded-lg border border-gray-200 p-3">
				<h2 class="flex justify-between text-sm font-semibold"><span>{date(day)}</span><span class="text-gray-400">{weekday(day)}</span></h2>
			</div>
		{/each}
	</div>
